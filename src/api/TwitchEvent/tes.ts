...
export class TES {
    ...
    private port: number;
    private whserver: any;
    private _whserverlistener: any;
    ...
    constructor(config) {
        ...
        if (
            ...
            throw new Error(
                "TES identity config must contain either 'onAuthenticationFailure' or 'refreshToken'"
            );
        }
     ...
     * ```
     */
    getSubscriptions(cursor = undefined) {
        logger.debug(
            `Getting ${
                cursor
                    ? `subscriptions for cursor ${cursor}`
                    : "first page of subscriptions"
            }`
        );
        return this._getSubs(
            `${SUBS_API_URL}${cursor ? `?after=${cursor}` : ""}`
        );
    }
    ...
    getSubscriptionsByType(type, cursor) {
        ...
        return this._getSubs(
            `${SUBS_API_URL}?${`type=${encodeURIComponent(type)}`}${
                cursor ? `&after=${cursor}` : ""
            }`
        );
    }
    ...
    getSubscriptionsByStatus(status, cursor) {
        ...
        return this._getSubs(
            `${SUBS_API_URL}?${`status=${encodeURIComponent(status)}`}${
                cursor ? `&after=${cursor}` : ""
            }`
        );
    }
    ...
    async getSubscription(idOrType, condition) {
        if (condition) {
            logger.debug(
                `Getting subscription for type ${idOrType} and condition ${TESUtils.printObject(
                    condition
                )}`
            );
        } else {
        ...
        const getUntilFound = async (cursor) => {
            ...
            sub = data.find((s) => {
                if (condition) {
                    return (
                        s.type === idOrType &&
                        TESUtils.objectShallowEquals(s.condition, condition)
                    );
                } else {
    ...
    async subscribe(
        ...
        logger.debug(
            `Subscribing to topic with type ${type} and condition ${TESUtils.printObject(
                condition
            )}`
        );
        const token = await this.auth.getToken();
        ...
        if (data.data) {
            if (this.transportType === "webhook") {
                return new Promise((resolve, reject) =>
                    this.eventManager.queueSubscription(data, resolve, reject)
                );
            } else {
                const subscription = data.data[0];
                this.wsclient.addSubscription(
    ...
    async unsubscribe(
        ...
        if (condition) {
            logger.debug(
                `Unsubscribing from topic with type ${idOrType} and condition ${TESUtils.printObject(
                    condition
                )}`
            );
            let id: string;
            ...
            if (id) {
                ...
                throw new Error(
                    "subscription with given type and condition not found"
                );
            }
        } else {
    ...
    on(type: any, callback: any) {
        ...
        this.eventManager.addListener(type, callback);
    }

    async _getSubs(url: string | URL | Request) {
        const token = await this.auth.getToken();
        const headers = {
            "client-id": this.clientID,
            Authorization: `Bearer ${token}`,
        };
        return RequestManager.request(url, { headers }, undefined, this.auth);
    }

    static ignoreInMiddleware(middleware) {
        return (req, res, next) => {
            return req.path === "/teswh/event"
                ? next()
